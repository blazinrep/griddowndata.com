...
| **Knowledge Vault** | Official ZIM set (locked 2026-07-20): WikiMed medical, Ready.gov, iFixit, full Wikipedia no-pic, Wikibooks (~62.5 GB) + all-50-states OsmAnd maps (~18 GB) + checklists | Drive directories 01–03 | ✅ ZIMs staged; 85 map regions staged; Kiwix + OsmAnd~ reader APKs staged & signature-verified on the master drive |
...
- **Offline AI advisor (PARKED — do NOT start until v1 ships: prices set, payments live, one drive sold & verified).** Idea: an on-device LLM that runs 100% offline on the buyer's phone, ideally answering *from* the GridDownData corpus (retrieval/RAG) rather than its own memory — "the drive that talks you through it with the grid down." Could anchor a premium tier.
  - *Feasibility (researched 2026-07-21):* real today. Apps run a single GGUF model file fully offline. **App:** PocketPal AI (free/OSS, has a RAM filter) or MLC Chat (Play-Store, pre-optimized). **Model:** small 1–3B, 4-bit (Llama 3.2 1B/3B, Gemma 2 2B, Qwen 2.5 1.5–3B) ≈ 1–2 GB. On a ~4 GB budget phone (e.g. the test Moto G Stylus 5G) expect *usable but slow*.
  - *Licensing (matters because we sell drives):* apps are free/OSS. Models differ — **Qwen 2.5 (Apache-2.0)** and **Phi-3 (MIT)** are redistribution-friendly; Llama 3.2 / Gemma allow commercial use but with their own terms. Prefer Qwen/Phi if ever bundling on a product.
  - *Integration design (important):* `START_HERE.html` is a static `file://` page and cannot call another app, so today the LLM would be a *separate app* beside the dashboard. Deep integration (chat inside the dashboard) requires running the model as a localhost server the dashboard JS `fetch()`es — a build project. **Design law:** the curated dashboard/manual stays the safety source of truth; the LLM is only an explain/search helper, never the authority (hallucination risk in medical/safety moments). Keeping them separate is a feature, not a limitation.
...
| 2026-07-21 | **Vault Upload UI** added to `vault.html` (categorize Property/Insurance/Legal-Medical, camera/storage upload, in-page image/PDF preview, search — GDDV-1 crypto untouched; 7/7 crypto checks pass). **Energy & Fuel Calculator** built (`energy-fuel-calculator.html`, 3 modules: generator runtime, vehicle range [ICE gas/diesel — corrected from EV-centric to match ~98% of US fleet], off-grid solar/battery; verified vs hand-worked math) + featured tile + search routing wired into `START_HERE.html`. **Android app-handoff**: dashboard now intercepts `.zim`→Kiwix and `.obf`→OsmAnd links with an on-brand "open in app" overlay (jsdom-verified); `OPEN-THE-WIKIS-ON-ANDROID.html` readme added. **GAP-2 CLOSED**: reader APKs staged on the master drive & signature-verified — Kiwix `org.kiwix.kiwixmobile.standalone` (v5231765, IzzyOnDroid, dev-signed) and OsmAnd~ `net.osmand.plus` (v531003, F-Droid). Hardened `fetch-android-apps.sh` verifies each APK's sha256 + signing certificate against the repo index (fail-closed; built-in v1/v2/v3 signer reader, no SDK). `LICENSES/` folder added (GPLv3 text + written source offer) for redistribution compliance. |
| 2026-07-21 (b) | **Phone bring-up validated end-to-end**: on the test Moto G Stylus 5G (Android 12), Kiwix installed and opened a `.zim` straight off the SD card — the offline stack works on-device as designed. Notes for buyer docs: `.zim`→Kiwix / `.obf`→OsmAnd (readers are separate apps); Android file-picker→SD-card navigation is the main friction point; latest OsmAnd may report "not compatible" on older Android → recommend a known-good older build. `update-a-drive.sh` added (safe incremental rsync to refresh an already-built card; auto-finds the card, refuses if none mounted). **Offline on-device LLM researched and PARKED to §6 Later** (post-launch only) — see roadmap entry for app/model/licensing/integration findings. |
